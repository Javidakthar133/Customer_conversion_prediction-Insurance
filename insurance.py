...
st.set_page_config(page_title="Loan Attraction Predictor", page_icon=r"C:\Users\Javid Akthar\Desktop\Insurance\Home, insurance, loan, protection icon - Download on Iconfinder.jpeg",layout='wide')

with open(r'C:\Users\Javid Akthar\Desktop\Insurance\onehot_encoder.pkl', 'rb') as f:
    loaded_encoder_ohc = pickle.load(f)


st.markdown("<h1 style='text-align:center;text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5); color:#4B3B40; font-size:50px;'>Customer Loan Conversion Predictor | By Javid Akthar</h1>", unsafe_allow_html=True)

background_image_url = "https://png.pngtree.com/background/20231218/original/pngtree-personal-loan-word-cloud-exchange-investment-personal-photo-picture-image_6874870.jpg"  
...
selected = option_menu(
    ...
    styles={
        "nav-link-selected": {
            "background-color": "#6D5C5C",  # Highlighted option with blue
    ...
    with st.form("Classification"):
        ...
        if submitted:
            ...
            try:
                ...
                with open(r'C:\Users\Javid Akthar\Desktop\Insurance\xgboost_model.pkl','rb') as file:
                ...
                with col2:
                    if outcome == 1:
                        st.markdown(f'<h2 class="Tail">Prediction is: {Outcome}</h2>', unsafe_allow_html=True)
                    else:
                        st.markdown(f'<h2 class="Tail">Prediction is: {Outcome}</h2>', unsafe_allow_html=True)
